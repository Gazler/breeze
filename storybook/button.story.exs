defmodule Breeze.Storybook.Stories.Blocks.ButtonStory do
  ...
  def story do
    %{
      ...
      source: ~s|<.button id="confirm" class="w-10">Confirm</.button>|
    }
  end
  ...
  def render(assigns) do
    ...
    <box class="w-full bg-panel">
      ...
      <box class="pt-1 grid grid-cols-3 gap-x-1">
        <.button id="storybook-button-primary" class="w-10">Confirm</.button>
        <.button id="storybook-button-cancel" class="w-10">Cancel</.button>
        <.button id="storybook-button-delete" class="w-10 bg-error">Delete</.button>
      </box>
    </box>
    """
  end
