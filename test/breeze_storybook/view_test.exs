defmodule Breeze.Storybook.ViewTest do
  ...
  test "F3 cycles the storybook theme" do
    ...
    assert metadata.theme.name == "nebula"
    assert metadata.assigns.theme_id == :nebula
    assert metadata.assigns.theme_label == "Nebula"

    assert {:ok, _acc, _box} = Breeze.ChildServer.render(pid, terminal: terminal)
    assert Breeze.ChildServer.metadata(child).theme.name == "nebula"
  end
  ...
  test "F3 cycles the storybook theme while focus is inside the preview" do
    ...
    assert Breeze.ChildServer.metadata(pid).theme.name == "nebula"
  end
  ...
  test "server lets storybook F3 run before focused preview live child input" do
    ...
    wait_until(fn ->
      Breeze.ChildServer.metadata(view_pid).theme.name == "nebula"
    end)
  end
